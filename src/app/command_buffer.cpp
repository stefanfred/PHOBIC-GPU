...
void CommandBuffer::pipelineBarrier(vk::PipelineStageFlags srcStageMask,
    ...
    primaryBuffer.pipelineBarrier(srcStageMask, dstStageMask, dependencyFlags, memoryBarriers, bufferMemoryBarriers,
                                  imageMemoryBarriers);
}

void CommandBuffer::readWritePipelineBarrier() {
    pipelineBarrier(vk::PipelineStageFlagBits::eAllCommands, vk::PipelineStageFlagBits::eAllCommands,
                    vk::DependencyFlags(),
                    {vk::MemoryBarrier(vk::AccessFlagBits::eShaderWrite, vk::AccessFlagBits::eShaderRead)}, {}, {});
}

void CommandBuffer::bindComputeDescriptorSet(const vk::PipelineLayout &layout, const vk::DescriptorSet &set,
...
// overloads for convenience
void CommandBuffer::bindComputeDescriptorSet(const Pipeline &pipeline, const DescriptorSetAllocation &set,
                                             const uint32_t setNumber) {
    bindComputeDescriptorSet(pipeline.layout(), set.set, setNumber);
}
...
void CommandBuffer::readTimestamps(const vk::Device &device, const vk::PhysicalDevice &pDevice) {
    ...
    CHECK(device.getQueryPoolResults(timestampPool, 0, timestampsInfo.size(), timestampsInfo.size() * sizeof(uint64_t),
    ...
    for (uint32_t i = 0; i < timestampsInfo.size(); i++) {
        res.push_back(TimestampResult{TimestampHandle{timestampsInfo.get(i), i},
                                      properties.limits.timestampPeriod * (timestamps[i] - timestamps[0])});
    }
    lastRead = res;
    delete[] timestamps;
}
