...
public:
    BuildInvocation(Mphf &f, const std::vector<Key> &keys, uint32_t size, MPHFconfig config, App &app,
                    MPHFbuilder *builder) : f(f), keys(keys), size(size), config(config), app(app), builder(builder) {
        size = keys.size();
    ...
    void addGpuCommands() {
        ...
        builder->bucketSizesStage.addCommands(cb, {size, partitions, config.bucketCountPerPartition}, keysSrc.buffer,
    ...
    bool run() {
        ...
        fillDeviceWithStagingBufferVec(app.pDevice, app.device, app.transferCommandPool, app.transferQueue, keysSrc,
                                       keys);
        fillDeviceWithStagingBufferVec(app.pDevice, app.device, app.transferCommandPool, app.transferQueue, fulcrums,
                                       config.getFulcs());
